...
var getPoyntURL = function getPoyntURL(env, region, app) {
  if (app === "WEB") {
    if (env === "dev") {
      ...
      return "https://poynt.dev-godaddy.com";
    } else if (env === "ci") {
      return "https://ci.poynt.net";
    } else if (env === "st") {
      return "https://st.poynt.net";
    } else if (env === "test") {
      return "https://poynt.test-godaddy.com";
    } else if (env === "ote") {
      return "https://poynt.ote-godaddy.com";
    } else if (region === "eu") {
      return "https://eu.poynt.net";
    } else {
 ...
 * @param {String} options.customUserAgentPrefix (optional) - custom prefix to add to User-Agent header
...
module.exports.nakedRequest = function nakedRequest(options, next) {
  ...
  // set request headers
  if (options.requestId && !validator.isUUID(options.requestId)) {
    return next(module.exports.error({
      name: "InvalidArgumentError",
      message: "Invalid UUID format for requestId: " + options.requestId,
      statusCode: 400,
    }));
  }
  ...
  if (options.customUserAgentPrefix) {
    headers["User-Agent"] = options.customUserAgentPrefix + "-" + headers["User-Agent"];
  }
  ...
  // build request url + fetch options
  var url = getPoyntURL(options.env, options.region, options.app) + options.url;
  var fetchOptions = {
  ...
  debug(
    "Poynt API request:",
    JSON.stringify({ url: url, method: fetchOptions.method, headers: headers, body: fetchOptions.body })
  );

  return fetch(url, fetchOptions)
    .then(function (res) {
      return res.text().then(function (body) {
        debug("Poynt API response:", res.status, body);
        ...
        if (typeof body === "string" && options.form) {
          ...
          try {
            ...
            // leave body as raw text, matching the previous json:true fallback behavior
          }
        }
        ...
        if (!requestErr.httpStatus) {
          requestErr.httpStatus = res.status;
        }

        // api request error
        next(module.exports.error(requestErr));
      });
    })
    .catch(function (err) {
      // network-level error (including a timeout abort) - no response was received
      next(module.exports.error(err));
    });
};
